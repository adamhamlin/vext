...
describe('toggleCommentType', () => {
                ...
                if (mode === testModes.HIGHLIGHTED_SELECTION) {
                    ...
                    switch (mode) {
                        ...
                        case testModes.CURSOR_MIDDLE_LINE: {
                            const newLineNumber = Math.ceil((currentSelection.end.line - currentSelection.start.line) / 2);
            ...
            describe('Parsing edge cases', () => {
                ...
                if (mode === testModes.CURSOR_FIRST_LINE) {
                    ...
                    it('reach maximum block comment probe depth, treat as free text', async () => {
                        ...
                        const editor = await openEditorWithContentAndSelectAll('javascript', [
                            '/**',
                            _.times(151, (num) => ` * Line ${num}`).join('\n'),
                            ' */'
                        ].join('\n'));
                        await toggleCommentTypeAndValidateText(editor, [
                            '// /**',
                            _.times(151, (num) => `//  * Line ${num}`).join('\n'),
                            '//  */'
                        ].join('\n'));
    ...
    describe('auto formatting configuration setting', () => {
        ...
        it('Rewrap formatter is invoked', async () => {
            const stub = sinon.stub(dependencyCommand, 'invokeDependencyCommand')
